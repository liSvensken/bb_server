...
export const step4HashPassword = ((callback: (err: ErrorInterface, statusCode: number, nowStepsResults: StepsResultRegistration) => void,
                                   userPassword: string, stepsResults: StepsResultRegistration) => {

  hashPassword((err, statusCode, result) => {
    if (!err) {
      stepsResults.step4HashPassword = result;
